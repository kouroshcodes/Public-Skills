...
if [ "$FROM" = lead ]; then
  pane=$(wezterm cli spawn --cwd "$REPO" -- zsh -lc \
    "exec claude --name wc$CHAIN-lead${GEN:+-$GEN} ${LEAD_CLAUDE_FLAGS:---permission-mode auto --model opus --autocompact 450k} '/wave-chain --implement'")
...
for k in $(seq "$FROM" "$TO"); do
  pane=$(wezterm cli spawn --cwd "$REPO" -- zsh -lc \
    "exec claude --name wc$CHAIN-wave$k ${WAVE_CLAUDE_FLAGS:---permission-mode auto --model sonnet --autocompact 600k} '/wave-chain --implement $k'")
